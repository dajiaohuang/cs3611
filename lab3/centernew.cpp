        ...
        if(select_rv == -1){
            ...
            if(select_rv == 0){
                ...
                if(FD_ISSET(sockfd, &readfds)){
					...
					ss<<buf;
                    std::ss >> head;// "To "
                    std::ss >> WhichToSend; //e.g."h1:"
                    getline(std::ss,WhatToSend);//e.g."Hello!"
	                ...
	                freeaddrinfo(servinfo1); 
                     ...
                     if (send(new_fd, WhatToSend+" From "+IP[inet_ntop(remoteaddr.ss_family,
								get_in_addr((struct sockaddr*)&remoteaddr),
								remoteIP, INET6_ADDRSTRLEN)].data(), WhatToSend.length()+7, 0) == -1) {
                         perror("send");
                     }
